...
def _panels(summaries: list[Summary], figure: dict[str, Any]) -> list[tuple[str, list[Summary]]]:
    ...
    for summary in summaries:
        ...
        panel_key = canonical_json(summary.labels[panel]) if panel else ""
        panel_label = (
...
def plot(config: ExperimentConfig, output_dir: str | Path) -> list[Path]:
    ...
    prepared = _prepare_figures(figures, summaries)

    current = read_json(cache.current_path)
    paths: list[Path] = []
    cache_keys = []
    for figure, selected, name in prepared:
        ...
        render = partial(
            _render_figure, summaries=selected, figure=figure, name=name, plotter=plotter
        )

        directory = cache.publish("plots", identity, render)
        filenames = read_json(directory / "figures.json")["files"]
        paths.extend(cache.export(directory, filenames, figures=True))
...
def _prepare_figures(
    figures: list[dict[str, Any]], summaries: list[Summary]
) -> list[tuple[dict[str, Any], list[Summary], str]]:
    prepared = []
    names: set[str] = set()
    for index, figure in enumerate(figures):
        ...
        name = safe_name(figure.get("name", f"{metric}-{index + 1}"), "Figure name")
        if name in names:
...
def _render_figure(
    ...
    *,
    ...
    """Render one figure and verify every returned file belongs to its generation."""
    formats = figure.get("formats", ["pdf"])
    generated = []
    if plotter is not None:
        generated.extend(Path(path) for path in plotter.plot(summaries, figure, target))
    else:
        if any(fmt in {"pdf", "jpg"} for fmt in formats):
            generated.extend(_matplotlib(summaries, figure, target, name))
        if "tikz" in formats:
            path = target / f"{name}.tikz"
            path.write_text(_tikz(summaries, figure), encoding="utf-8")
    ...
    for path in generated:
        ...
        try:
            ...
            raise ValueError("Custom plotters must write inside their output directory") from exc
        if not path.is_file():
...
def _coordinates(x: np.ndarray, values: np.ndarray) -> str:
    return " ".join(
        f"({float(coordinate):.17g},{float(value):.17g})" for coordinate, value in zip(x, values)
    )
